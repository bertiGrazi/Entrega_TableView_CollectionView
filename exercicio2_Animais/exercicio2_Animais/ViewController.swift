...
extension ViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if let viewController = UIStoryboard(name: "AnimalsDetail", bundle: nil).instantiateInitialViewController() as? AnimalsDetailViewController{
            viewController.name = arrayAnimals[indexPath.row].name
            viewController.image = arrayAnimals[indexPath.row].image
